	...
	return (
		...
		<div className={"bg-white px-3 pb-3 rounded-xl min-w-[300px] max-w-[1000px]" + space + props.className} style={props.style}>
			...
			<div className="flex justify-between items-center">
				...
				<div className='flex justify-between items-center'>
					<BiCopy className='relative translate-y-2 text-3xl hover:scale-110 duration-200 gc-text-green' onClick={props.onCopy} />
					{props.admin===true && <MdDeleteForever className='relative translate-y-2 text-3xl hover:scale-110 duration-200 mx-3 text-red-500' onClick={props.onDelete} />}
					{props.admin===true && <FiEdit className='relative translate-y-2 text-2xl hover:scale-110 duration-200' onClick={props.onEdit} />}
				</div>
			</div>
